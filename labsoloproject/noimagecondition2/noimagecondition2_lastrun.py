...
expName = 'labsoloproject'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\User\\Desktop\\AU\\SEM5\\PSY310\\labsoloproject\\noimagecondition2\\noimagecondition2_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instruction1 = visual.TextStim(win=win, name='instruction1',
        text='Instructions:\nPlease identify whether the following words are nouns or verbs and type the response as "noun" or "verb."\n\nOnce done, click on \'Submit\'.\n\nPlease click the \'spacebar\' to proceed.\n',
        font='Arial',
    ...
    # --- Initialize components for Routine "semantic_processing" ---
    ...
    instruction2 = visual.TextStim(win=win, name='instruction2',
        text="This is a surprise recall test. Please write as many words as you can recall. Write the words with commas in between them (XXX, YYY and so on). You can take a maximum of 90 seconds for this task.\n\nOnce done, please click on 'Submit'.\n\nPlease click on the 'spacebar' once you are ready.",
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Initialize components for Routine "recalltest" ---
    ...
    thankyoutext = visual.TextStim(win=win, name='thankyoutext',
        text='The experiment is over. Thank you for your time.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # create an object to store info about Routine phase1
    phase1 = data.Routine(
        name='phase1',
        components=[instruction1, key_resp_2],
    )
    ...
    key_resp_2.rt = []
    _key_resp_2_allKeys = []
    # store start times for phase1
    phase1.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    phase1.tStart = globalClock.getTime(format='float')
    phase1.status = STARTED
    thisExp.addData('phase1.started', phase1.tStart)
    phase1.maxDuration = None
    # keep track of which components have finished
    phase1Components = phase1.components
    for thisComponent in phase1.components:
    ...
    # --- Run Routine "phase1" ---
    phase1.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        ...
        if not continueRoutine:  # a component has requested a forced-end of Routine
            phase1.forceEnded = routineForceEnded = True
            break
    ...
    # store stop times for phase1
    phase1.tStop = globalClock.getTime(format='float')
    phase1.tStopRefresh = tThisFlipGlobal
    thisExp.addData('phase1.stopped', phase1.tStop)
    # check responses
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('noimagecondition2.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "semantic_processing" ---
        # create an object to store info about Routine semantic_processing
        semantic_processing = data.Routine(
            name='semantic_processing',
            components=[fixation, text_2, textbox, button_2],
        )
        semantic_processing.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        text_2.setText(word)
        textbox.reset()
        ...
        button_2.reset()
        # store start times for semantic_processing
        semantic_processing.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        semantic_processing.tStart = globalClock.getTime(format='float')
        semantic_processing.status = STARTED
        thisExp.addData('semantic_processing.started', semantic_processing.tStart)
        semantic_processing.maxDuration = None
        # keep track of which components have finished
        semantic_processingComponents = semantic_processing.components
        for thisComponent in semantic_processing.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "semantic_processing" ---
        # if trial has changed, end Routine now
        if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
            continueRoutine = False
        semantic_processing.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            if textbox.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if button_2.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                semantic_processing.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in semantic_processing.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "semantic_processing" ---
        for thisComponent in semantic_processing.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for semantic_processing
        semantic_processing.tStop = globalClock.getTime(format='float')
        semantic_processing.tStopRefresh = tThisFlipGlobal
        thisExp.addData('semantic_processing.stopped', semantic_processing.tStop)
        ...
        # the Routine "semantic_processing" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
    ...
    # create an object to store info about Routine phase2
    phase2 = data.Routine(
        name='phase2',
        components=[instruction2, key_resp],
    )
    ...
    key_resp.rt = []
    _key_resp_allKeys = []
    # store start times for phase2
    phase2.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    phase2.tStart = globalClock.getTime(format='float')
    phase2.status = STARTED
    thisExp.addData('phase2.started', phase2.tStart)
    phase2.maxDuration = None
    # keep track of which components have finished
    phase2Components = phase2.components
    for thisComponent in phase2.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "phase2" ---
    phase2.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            phase2.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in phase2.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in phase2.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for phase2
    phase2.tStop = globalClock.getTime(format='float')
    phase2.tStopRefresh = tThisFlipGlobal
    thisExp.addData('phase2.stopped', phase2.tStop)
    # check responses
    ...
    # --- Prepare to start Routine "recalltest" ---
    # create an object to store info about Routine recalltest
    recalltest = data.Routine(
        name='recalltest',
        components=[textbox_2, button],
    )
    recalltest.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    textbox_2.reset()
    # reset button to account for continued clicks & clear times on/off
    button.reset()
    # store start times for recalltest
    recalltest.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    recalltest.tStart = globalClock.getTime(format='float')
    recalltest.status = STARTED
    thisExp.addData('recalltest.started', recalltest.tStart)
    recalltest.maxDuration = None
    # keep track of which components have finished
    recalltestComponents = recalltest.components
    for thisComponent in recalltest.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "recalltest" ---
    recalltest.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 90.0:
        ...
        if textbox_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > textbox_2.tStartRefresh + 90.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if button.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > button.tStartRefresh + 90.0-frameTolerance:
                # keep track of stop time/frame for later
                button.tStop = t  # not accounting for scr refresh
                button.tStopRefresh = tThisFlipGlobal  # on global time
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            recalltest.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in recalltest.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "recalltest" ---
    for thisComponent in recalltest.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for recalltest
    recalltest.tStop = globalClock.getTime(format='float')
    recalltest.tStopRefresh = tThisFlipGlobal
    thisExp.addData('recalltest.stopped', recalltest.tStop)
    ...
    if recalltest.maxDurationReached:
        routineTimer.addTime(-recalltest.maxDuration)
    elif recalltest.forceEnded:
        routineTimer.reset()
    else:
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        if thankyoutext.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > thankyoutext.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
